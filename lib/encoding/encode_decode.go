...
func Encode(datas [][]int64, pubKey kyber.Point, signatures [][]libdrynx.PublishSignature, ranges []*libdrynx.Int64List, operation libdrynx.Operation) ([]libunlynx.CipherText, []int64, []libdrynxrange.CreateProof) {
	...
	case "lin_reg":
		if withProofs {
			encryptedResponse, clearResponse, createPrf = EncodeLinearRegressionDimsWithProofs(datas, pubKey, signatures, ranges)
		} else {
			encryptedResponse, clearResponse = EncodeLinearRegressionDims(datas, pubKey)
		}
	case "frequencyCount":
		if withProofs {
